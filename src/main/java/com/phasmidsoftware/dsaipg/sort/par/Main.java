...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            FileOutputStream fis = new FileOutputStream("/Users/nikhilpandey/PSA_ProfessorRepo/result.csv");
    ...
    private static void processCommand(String x, String y) {
        if (x.equalsIgnoreCase("N")) {
            // Set configuration values specified by 'N'
            setConfig(x, Integer.parseInt(y));
        } else if (x.equalsIgnoreCase("P")) {
            // Retrieve and display the current common pool parallelism level
            ...
            // Set the cutoff value for the parallel sorting algorithm
            try {
    ...
    /**
     * Performs sorting tests over a range of array sizes and cutoff values.
     * This method tests how the sorting algorithm performs with different sizes of arrays and varying cutoffs for parallel sorting.
     * The tests are designed to average the execution time over multiple runs for stability and accuracy of results.
     */
    private static void performSortingTests() {
        Random random = new Random();
        int[] sizes = {100000, 500000, 1000000, 2000000, 5000000}; // Array sizes to test
        ...
        for (int size : sizes) {
            ...
            for (int j = 50; j < 100; j++) {
                ...
                timeList.add(totalTimes / 10); // Average time for better stability in results
